...
# load dataset 載入資料
dataset = read_csv('./1A-test(2013-2016).csv', header=0, index_col=0)#呼叫檔案，從第一列開始header=0,從第一行開始 index_col=0
...
reframed.drop(reframed.columns[[11,12,13,14,15,16,17,18,19]], axis=1, inplace=True) #刪除不要使用的數據，保留預測數據
    ...
    for e in epochs:
        for b in batch_size:
            ...
            for i in range(1,31,1):
                ...
                # design network 架設模型
                model = Sequential()                
                model.add( Bidirectional(LSTM(n, input_shape=(train_X.shape[1], train_X.shape[2]),return_sequences = False)))#,return_sequences = False,activation = 'relu'
